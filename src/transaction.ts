...
const ACTIVATE_STREAK_FUNCTION_SELECTOR = "0x31711309";
const MAX_RETRIES = 3;
...
function encodeActivateStreakCall(address: string): string {
  const cleanAddress = address.startsWith('0x') ? address.substring(2) : address;
  return `${ACTIVATE_STREAK_FUNCTION_SELECTOR}000000000000000000000000${cleanAddress}`;
}

async function estimateGas(walletAddress: string, rpcUrl: string): Promise<bigint> {
  try { 
    logger.info(`Estimating gas...`);
    const callData = encodeActivateStreakCall(walletAddress);
...
export async function sendActivateStreakTransaction(): Promise<string> {
  let attemptCount = 0;
  ...
  while (attemptCount < MAX_RETRIES) {
    for (const rpc of endpointsToTry) {
      try {
        ...
        const callData = encodeActivateStreakCall(wallet.address);
        ...
        logger.info(`Preparing to call activateStreak() on contract: ${config.contractAddress}`);
        ...
        try {
          ...
          if (errorMsg.includes("execution reverted")) {
            ...
            logger.warn("The contract may have time restrictions or other conditions for the activateStreak() function.");
          } else {
